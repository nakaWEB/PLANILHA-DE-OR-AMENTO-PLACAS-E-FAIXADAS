...
// ======= VARIÁVEIS GLOBAIS =======
const nomesPaginas = [
    "CLIENTE", "SERVIÇOS GERAIS/COMPOSTOS",
    "ADESIVOS/BANNERS", "PVC/MDF/ACRÍLICO",
    "MÃO DE OBRA", "ORÇAMENTO", "TOTAL GERAL"
];
...
let catalogoProdutos = {
    "SERVIÇOS GERAIS/COMPOSTOS": {
        "CARTÕES (beta)": [
            {
                tipo: "composto", codigo: "456345634654356", idX3D: "645634534656", nome: "CARTÃO DE VISITA", preco: 0, formula: "Q * ($[VCARTAO500] + $[FCARTAO500] + $[TSCARTAO500])", unidade: "500-UNID",
                ...
                componentes: [
                    { codigo: "VCARTAO500", nome: "VALOR + 40%", preco: 84, idX3D: "R$ 60,00" },
                    { codigo: "MLCARTAO500", nome: "MARGEM DE LUCRO", preco: 0, idX3D: "40%" },
                    { codigo: "FCARTAO500", nome: "FRETE", preco: 30, idX3D: "R$ 30,00" },
                    { codigo: "TSCARTAO500", nome: "T. SERVIÇO", preco: 25, idX3D: "R$ 25,00" }
                ]
            }
        ]
    },
    ...
    "ADESIVOS/BANNERS": {
        ...
        "ADESIVOS": [
            ...
            { codigo: "2426682820", idX3D: "022", nome: "ADESIVO RECORTE", preco: 7, formula: "A * C * Q *$", unidade: "M²" },
    ...
    "PVC/MDF/ACRÍLICO": {
        ...
        "MDF": [
            ...
            { codigo: "4251", idX3D: "2601110", nome: "RODÍZIO 35MM S FREIO", preco: 16.00, formula: "Q * $", unidade: "UNID" },
            { codigo: "93582", idX3D: "", nome: "RODÍZIO 50MM S FREIO", preco: 18.00, formula: "Q * $", unidade: "UNID" },
    ...
    "MÃO DE OBRA": {
        "ENTREGA E INSTALAÇÃO": [
            { codigo: "1522263717", idX3D: "2600885", nome: "ALMOÇO/LANCHE", preco: 40.00, formula: "Q * $", unidade: "UNID" },
            { codigo: "2058430340", idX3D: "2601119", nome: "ALUGUEL DE ANDAIME", preco: 3.00, formula: "Q * $", unidade: "PÇ" },
            { codigo: "1384869812", idX3D: "2600846", nome: "FRETE", preco: 3.00, formula: "Q * $", unidade: "H" },
            { codigo: "2268669278", idX3D: "", nome: "TEMPO DE PRODUÇÃO (ESTRADA)", preco: 0.83, formula: "Q[35538] / (0.6 * 60) * 2 * 15", unidade: "H" },
            { codigo: "3721873499", idX3D: "", nome: "IMPRESSÃO A4 COLORIDA", preco: 2.50, formula: "Q * $", unidade: "UNID" },
            { codigo: "1397703156", idX3D: "630", nome: "TEMPO INSTALAÇÃO COMUNICAÇÃO VISUAL", preco: 25.00, formula: "Q * $", unidade: "H" },
        ],

        "ATENDIMENTO E DESIGNER": [
        ...
        "PRODUÇÃO": [
            { codigo: "1885468949", idX3D: "43822", nome: "TEMPO DE LASER - MATERIAL CLIENTE (VERIFICAR NO PROGRAMA DE CORTE)", preco: 1.00, formula: "C * Q * $", unidade: "H" },
            { codigo: "3219641335", idX3D: "43822", nome: "TEMPO DE LASER (VERIFICAR NO PROGRAMA DE CORTE)", preco: 15, formula: "(C / 60 * Q) * $", unidade: "H" },
            { codigo: "3111476538", idX3D: "630", nome: "TEMPO DE COPOS E CANECAS", preco: 25.00, formula: "Q * $", unidade: "H" },
            ...
            { codigo: "3297", idX3D: "143", nome: "TEMPO PRODUÇÃO PRESTAÇÃO DE SERVIÇO", preco: 25.00, formula: "Q * $", unidade: "H" },
            { codigo: "23029486", idX3D: "", nome: "VALOR PARA ARREDONDAR", preco: 1, formula: "Q * $", unidade: "UNID" },
        ],
    },
};
...
function criarLinhaProdutoComposto(prod) {
    ...
    return `
        <tr
            class="produto-composto"
            data-formula="${prod.formula || ''}"
            data-product-id="${prod.id || ''}">

            <td class="expandir-composto">
    ...
    if (dadosCategoria && typeof dadosCategoria === 'object' && !Array.isArray(dadosCategoria)) {
        ...
        subcategorias.forEach((sub, index) => {
            ...
            produtos.forEach(prod => {
                ...
                if (prod.tipo === "composto") {

                    linhas += criarLinhaProdutoComposto(prod);
...
function atualizarOrcamento() {
    ...
    // Percorre TODAS as linhas de produtos de TODAS as páginas
    document.querySelectorAll('.planilha tbody tr').forEach(linha => {
        ...
        const isComposto = linha.classList.contains("produto-composto");

        if (isComposto) {
            // === PRODUTO COMPOSTO: mostra apenas os COMPONENTES ===

            // Busca o produto no catalogoProdutos para obter os componentes
            ...
            for (const categoria in catalogoProdutos) {
                ...
                if (typeof grupo === "object" && !Array.isArray(grupo)) {
                    for (const sub in grupo) {
                        produtoBanco = grupo[sub].find(p => p.codigo === codigo);
                        if (produtoBanco) break;
                    }
                } else {
                    produtoBanco = grupo.find(p => p.codigo === codigo);
                }
                if (produtoBanco) break;
            }

            // Se encontrou o produto e tem componentes, adiciona cada componente
            if (produtoBanco && produtoBanco.componentes && produtoBanco.componentes.length > 0) {
                produtoBanco.componentes.forEach(comp => {
                    // Ignora componentes que começam com "ML" (se houver)
                    if (comp.codigo && comp.codigo.startsWith("ML")) return;

                    const valorComp = comp.preco * quantidade;

                    items.push({
                        codigo: comp.codigo,
                        nome: comp.nome,
                        quantidade: quantidade,
                        unidade: comp.unidade || "UNID",
                        valorTotal: valorComp
                    });
                    totalOrcamento += valorComp;
                });
            }
        } else {
            // === PRODUTO NORMAL: mostra o produto diretamente ===
            items.push({ codigo, nome, quantidade, unidade, valorTotal });
            totalOrcamento += valorTotal;
        }
    });

    // Ordena os itens por código
    ...
    const imposto = totalOrcamento * 0.14;
            ...
            if (linha.querySelector(".preco-base")) {
                ...
                if (formula && formula.trim() !== "") {
                    ...
                    try {
                        ...
                        if (linha.classList.contains("produto-composto")) {
                            ...
                            if (produtoBanco?.componentes) {
                                ...
                                produtoBanco.componentes.forEach(comp => {
                                    ...
                                    if (
                                        comp.codigo.startsWith("ML")
                                    ) return;

                                    soma += Number(comp.preco || 0);
...
document.addEventListener("click", e => {
    ...
    const botao =
        e.target.closest(".expandir-composto");
    ...
    componentes.style.display =
        componentes.style.display === "table-row"
            ? "none"
            : "table-row";
...
// ======= FUNÇÃO APLICAR PRODUTO NA TABELA - CORRIGIDA =======
function aplicarProdutoNaTabela({ codigo, nome, L, C, Q, unidade, precoBase, valorTotal }) {
    ...
    // 🔍 Busca TODAS as linhas de produtos (IGNORA linhas de componentes)
    const todasLinhas = Array.from(document.querySelectorAll('.planilha tbody tr:not(.linha-componentes)'));
    ...
    if (!linhaDisponivel) {
        ...
        const isComposto = ultima.classList.contains('produto-composto');

        if (isComposto) {
            // Clona produto composto (não tem .btn-clone, usa lógica diferente)
            linhaDisponivel = clonarLinhaComposto(ultima);
        } else {
            // Clona produto normal
            const btnClone = ultima.querySelector('.btn-clone');
            if (btnClone) {
                clonarLinha(btnClone);
    ...
    // ✅ Preenche a linha (VERIFICA se inputs existem antes de preencher)
    const inputLargura = linhaDisponivel.querySelector('.largura');
    const inputComprimento = linhaDisponivel.querySelector('.comprimento');
    const inputQuantidade = linhaDisponivel.querySelector('.quantidade');

    if (inputLargura) inputLargura.value = L || 0;
    if (inputComprimento) inputComprimento.value = C || 0;
    if (inputQuantidade) {
        inputQuantidade.value = Q;
        // Dispara evento input para recalcular
        inputQuantidade.dispatchEvent(new Event('input'));
    }

    // Se for produto composto e Q > 0, expande os componentes
    if (linhaDisponivel.classList.contains('produto-composto') && Q > 0) {
        const componentes = linhaDisponivel.nextElementSibling;
        if (componentes && componentes.classList.contains('linha-componentes')) {
            componentes.style.display = 'table-row';
            const icon = linhaDisponivel.querySelector('.expandir-composto i');
...
// ======= FUNÇÃO PARA CLONAR PRODUTO COMPOSTO (NOVA) =======
function clonarLinhaComposto(linhaOriginal) {
    // Clona a linha do produto composto
    const cloneProduto = linhaOriginal.cloneNode(true);

    // Reseta os inputs do clone
    cloneProduto.querySelectorAll('input').forEach(inp => {
        inp.value = 0;
    });
    cloneProduto.querySelector('.valor-total').textContent = '0,00';

    // Fecha o ícone de expansão no clone
    const icon = cloneProduto.querySelector('.expandir-composto i');
    if (icon) icon.className = 'fas fa-chevron-right';

    // Insere após a linha original
    linhaOriginal.after(cloneProduto);

    // Clona também a linha de componentes (se existir)
    const linhaComponentes = linhaOriginal.nextElementSibling;
    if (linhaComponentes && linhaComponentes.classList.contains('linha-componentes')) {
        const cloneComponentes = linhaComponentes.cloneNode(true);
        cloneComponentes.style.display = 'none';
        cloneProduto.after(cloneComponentes);
    }

    // Monitora os inputs da nova linha
    monitorarInputs();

    showNotification('Produto composto clonado com sucesso!', 'info');

    return cloneProduto;
}

// ======= FUNÇÃO CLONAR LINHA NORMAL (mantida) =======
